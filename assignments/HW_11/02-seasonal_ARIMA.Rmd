...
ects %>% features(log(ecompctnsa), list(unitroot_kpss, unitroot_nsdiffs, unitroot_ndiffs)) %>% knitr::kable()
```
...
```{r Run the stationarity tests again, warning=F}
tseries::adf.test(ects$sales.trans[!is.na(ects$sales.trans)])
ects %>% features(sales.trans, list(unitroot_kpss, unitroot_nsdiffs, unitroot_ndiffs)) %>% knitr::kable()
```
...
```{r Model identification and estimation, warning=F, echo=F}
...
fit %>% pivot_longer(everything(), names_to="Model name", values_to="Orders") %>% knitr::kable()
```

```{r}
glance(fit) |> arrange(AICc) |> select(.model:BIC) %>% knitr::kable()
```
...
```{r forcasting, echo=F, message=F}
fc <- fit |>
	fabletools::forecast(h=4)
```

```{r Forecast from auto model plot, echo=F, warning=F, message=F}
ects %>%
	autoplot(ecompctnsa) +
...
```{r Forecast from my model plot, echo=F, warning=F, message=F}
ects %>%
	autoplot(ecompctnsa) +
...
```{r Actual forecasts with CIs, echo=F}
fc %>% hilo() %>% filter(.model != 'arima014111') %>% select(-ecompctnsa) %>% knitr::kable()
```
...
fabletools::accuracy(fc, ects.orig) |>
	...
	arrange(.model) |>
	select(.model, .type, RMSE, MAE, MAPE, MASE, RMSSE) %>% knitr::kable()
```
