...
```{r load packages for time series linear model, warning=FALSE, message=FALSE, echo=F}
library(tidyverse)
#library(fpp3)
```

```{r load temperature data, message=FALSE, echo=F} 
...
```{r plot electricity and temperature, echo=F}
...
coef(summary(coint_reg)) %>% knitr::kable()
...
```{r plot residuals, echo=F}
m1 %>% gg_tsresiduals() + labs(title='Residuals for the linear model')
```
...
```{r produce a forecast with the model, echo=F}
temp.nd <- new_data(temp, n=2)
temp.nd$temperature <- c(15, 35)
fabletools::forecast(m1, temp.nd) %>% hilo() %>% knitr::kable()
```
