---
title: "Unit 7 Live Session (Solutions)"
output: 'pdf_document'  
...
**Monthly hosing sales time series shows strong seasonality each year and some solid cyclic behavior with a period of about 6–10 years.**

**Daily changes in dow joes index has no trend, seasonality, or cyclic behavior. There are random fluctuations that do not appear to be very predictable.**
...
**Since, in practice, it is much easier to deal with additive series $Yt=T_t+S_t+W_t$, we will transform the data by taking the logarithms**
...
a)- The the following plots display the simulations of $X_t$ for different values of $\beta$:
...
**From the plot, we can see that $X_t$ is a non-stationary series with a constant variance but a non-constant mean. And higher $\beta$ or the more pronounced the trend leads to the slower declines in the ACF.**

\newpage

b)- The following time series process is from simulations of $Y_t$ for different values of $\omega$:
...
\begin{align*}
\bigtriangledown_d y_t &= y_t - y_{t-d}\\
&= \beta_1 t - \beta_1 (t-d) + S_t -S_{t-d} + W_t - W_{t-d}\\
&= \beta_1 d+ W_t -\bigtriangledown_d W_t
\end{align*}
...
\bigtriangledown_d y_t &=  y_t - y_{t-d}\\ 
&= \beta_1 d+ W_t -\bigtriangledown_d W_t\\
&= \beta_1 d+ \bigtriangledown_d W_t - \beta_1 d-1 - \bigtriangledown_d W_{t-1}\\
&= \beta_1 + \bigtriangledown_{d,1}^{1,1} W_t
\end{align*}
...
**The fourth model with quadratic trend and seasonal variables seems to do a good job capturing both trend and seasonal movement in the data.**
...
$$E(x_{tk} \cdot u_t) = 0$$ 
 ...
 - This assumption limits how $\epsilon_t$ is related to the explanatory variables in other periods.
...
fit_linear_season %>% gg_tsresiduals()
...
**The third assumption is no perfect multicollinearity. Since R didn't report any warning or missing coefficients when we estimated the model, this assumption is violated**.
...
**Assumpton-5 is homoskedasticity. If we look at the residuals v.s fitted values to plot, we can see roughly the same variance throughout the range residuals, and there are no severe deviations from this assumption.**
...
- $SE$ includes two sources of variance or uncertainty in our forecasts. First is $\sigma^2$, the variance of the errors in the population, and it does not change with the sample size. Second, the sampling error in $\hat{y}$ arises because we have estimated the $\beta_0$ and $beta_1$. Because  $\beta_0$ and $beta_1$ have a variance proportional to $1/T$, the sampling error can be very small for large samples.
